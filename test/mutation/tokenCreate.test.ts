test('tokenCreate_success_00', () => {
  ...
  const { resolve: roomResolve } = roomCreate;
  const { tokenCreate } = require('../../schema/mutation/token/create');
  const { resolve: tokenResolve } = tokenCreate;
      ...
      .then((result) => {
        ...
        return tokenResolve(...args)
          .then((result) => {
            ...
            expect(result.roomId).toBe(newToken.roomId);
            expect(result.hash).toBe(expectHash);
            resolve();
          })
      })
  })
    ...
    .then((result) => {
      ...
      return tokenResolve(...args)
        .catch(e => expect(e.message)
          .toMatch(/^authentication failed error: room:/))
    })
});
