test('tokenCreate_success_00', () => {
  ...
  return (async () => {
    const roomArgs = [{}, newRoom];
    const result = await roomResolve(...roomArgs);
    ...
    const newToken = { roomId, password, };

    const tokenArgs = [{}, newToken];
    const tokenResult = await tokenResolve(...tokenArgs)
    const { timestamp } = tokenResult;
    const expectHash = Encrypt.sha256(password + timestamp);

    expect(tokenResult.roomId).toBe(newToken.roomId);
    expect(tokenResult.hash).toBe(expectHash);
  })();
});
...
test('tokenCreate_fail_00', () => {
  ...
  return (async () => {
    const roomArgs = [{}, newRoom];
    const roomResult = await roomResolve(...roomArgs)
    expect(roomResult.title).toBe(newRoom.title);
    expect(roomResult.description).toBe(newRoom.description);
    expect(roomResult.password).toBe(newRoom.password);

    const roomId = roomResult._id.toString();
    const invalidPassword = '1110tset#';
    const newToken = {
      roomId,
    ...
    const tokenArgs = [{}, newToken];
    return tokenResolve(...tokenArgs)
      .catch(e => expect(e.message).toMatch(/^authentication failed error: room:/))
  })()
});
...
test('tokenCreate_fail_00', () => {
  ...
  const invalidPassword = '1110tset#';
  const newToken = {
    roomId: roomIdDoesNotExist,
    password: invalidPassword,
  };

  const args = [{}, newToken];
  return tokenResolve(...args)
    .catch(e => expect(e.message)
      .toMatch(/^resource not found error: room:/))
});
...
test('tokenCreate_fail_01', () => {
  ...
  const invalidPassword = '#test0111';
  const newToken = {
    roomId: roomIdIsShort,
    password: invalidPassword,
  };
